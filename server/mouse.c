...
void send_mouse_to_socket(int touch_fd, int client_socket_fd, struct input_event *ev) {
    switch (ev->type) {
    case EV_REL:
        switch (ev->code) {
        case REL_X:
            if(mouse == true) {
                ...
                sprintf(buffer, "{mouse:1,x:%d,y:%d}", last_abs_mt_position_x, last_abs_mt_position_y);
