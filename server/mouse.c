...
void set_aim_mouse(int touch_fd, struct mt_touch *mt_touch, struct aim_mouse *aim_mouse, int *slot) {
    ...
    mt_touch_down(touch_fd, mt_touch, slot, aim_mouse->last_x, aim_mouse->last_y);
}
            ...
            switch(ev->code) {
                case REL_X:
                    if(mouse_socket->is_mouse == false) {
                        ...
                        if(aim_mouse->last_y > MAX_ABS_MT_POSITION_Y || aim_mouse->last_y < 0) {
                ...
                case REL_Y:
                    if(mouse_socket->is_mouse == false) {
                        ...
                        if(aim_mouse->last_x > MAX_ABS_MT_POSITION_X || aim_mouse->last_x < 0) {
                            reset_aim_mouse(touch_fd, mt_touch, aim_mouse, slot);
                        } else {
            ...
            switch(ev->code) {
                case REL_X:
                    if(mouse_socket->is_mouse) {
                        ...
                        sprintf(mouse_socket->buffer, "{mouse:1,x:%d,y:%d}", mouse_socket->last_x, mouse_socket->last_y);
                ...
                case REL_Y:
                    ...
                    if(mouse_socket->is_btn_left) {
                        mt_touch_move(touch_fd, slot, MAX_ABS_MT_POSITION_X - mouse_socket->last_x, 0);
                    }
                    break;
            }
            break;
                    ...
                    switch(ev->value) {
                        case 1:
                            if(mouse_socket->is_mouse) {
                                ...
                                mt_touch_down(touch_fd, mt_touch, slot, MAX_ABS_MT_POSITION_X - mouse_socket->last_x + 1, mouse_socket->last_y + 1);
                            }
                            break;
                        case 0:
                            if(mouse_socket->is_mouse) {
